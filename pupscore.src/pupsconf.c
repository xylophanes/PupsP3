/*-----------------------------------------------------------------------
    Purpose: simple tool to configure P3 Makefiles for different
             architectures.
...
{   int i,
    ...
    {  (void)fprintf(stderr,"\npupsconf version %s, (C) Tumbling Dice 2002-2019 (built %s %s)\n\n",PUPSCONF_VERSION,__TIME__,__DATE__);
    ...
    {  start = 3;
       ...
       (void)snprintf(config_target,SSIZE,"%s/%s",DEFAULT_CONFIGDIR,argv[2]);
    else
       (void)snprintf(config_target,SSIZE,"%s/%s",getenv("P3_CONFDIR"),argv[1]);
       
    if((cstream = fopen(config_target,"r")) == (FILE *)NULL)
    {  (void)fprintf(stderr,"pupsconf: cannot find configuration file for target %s (path is %s)\n",argv[start],config_target);
       (void)fflush(stderr);
