...
{   (void)fprintf(stderr,"lib hseaplib %s: [ANSI C]\n",PHEAP_VERSION);
    ...
    if(level > 1)
    {  (void)fprintf(stderr,"(C) 2003-2018 Tumbling Dice\n");
...
{   int i,
    ...
    {  
	  ...
	  flags = O_RDWR;
          ...
          /*-----------------------------------------*/
          /* Save heap map and prot modes (for CKPT) */ 
          /*-----------------------------------------*/
          ...
          if(htable[i].exists == FALSE)
          {
             ...
             {  (void)strdate(date);
                (void)fprintf(stderr,
                         "(%s %s (%d@%s:%s): %d bit persistent heap %d [%s], length 0x%010x created and attached at 0x%010x virtual\n",
          ...
          if(appl_verbose == TRUE)
	  {     (void)fprintf(stderr,
                              "(%s %s (%d@%s:%s): %d bit persistent heap %d (%s), length 0x%010x (segment 0x%010x) attached at 0x%010x virtual\n",
                                                                                                     date,appl_name,appl_pid,appl_host,appl_owner,
                                                                                                                                htable[i].ptrsize,
                                                                                                                                                i,
                                                                                                                                       map_f_path,
...
{   int  i;
    ...
    {  (void)strdate(date);
       (void)fprintf(stderr,
                     "%s %s (%d@%s:%s): persistent heap %04d (%-24s), length 0x%010x (segment 0x%010x) detached from 0x%010x virtual",
                                                                                         date,appl_name,appl_pid,appl_host,appl_owner,
                                                                                                                                 hdes,
                                                                                                                    htable[hdes].name,
...
#ifdef PHEAP_DEBUG
(void)fprintf(stderr,"msm_sbrk: EXTENDING MAPPED SEGMENT (by %d brk segments = 0x%010x bytes)\n",
                                                                                  n_brk_segments,
...
#ifdef PHEAP_DEBUG
(void)fprintf(stderr,"msm_sbrk: heap %04d (%-32s) extended by 0x%010x bytes\n",hdes,htable[hdes].name,size);
(void)fflush(stderr);
#endif /* PHEAP_DEBUG */
...
{   char size_str[SSIZE]   = "",
    ...
    if(_phobjectmap[hdes][h_index]->size != 0)
       (void)snprintf(size_str,SSIZE,"0x%010x bytes",_phobjectmap[hdes][h_index]->size);
    else
    ...
    (void)fprintf(stream,"    pointer size          :  %d bytes\n",    htable[hdes].ptrsize);
    (void)fprintf(stream,"    pointer configuration :  %s\n",          endian_str);
    (void)fprintf(stream,"    object location       :  0x%x virtual\n",(unsigned long int)_phobjectmap[hdes][h_index]->addr);
    (void)fprintf(stream,"    object size           :  0x%x bytes\n\n",_phobjectmap[hdes][h_index]->size);
...
{   int i,
    ...
    (void)fprintf(stream,"\n\n    Persistent object map for heap \"%-32s\" (at 0x%010x virtual)\n\n",
                                                                                 msm_hdes2name(hdes),
    ...
    {  if(_phobjectmap[hdes][i] != (void *)NULL)
       {  char size_str[SSIZE] = "";
          ...
          if(_phobjectmap[hdes][i]->size != 0)
             (void)snprintf(size_str,SSIZE,"0x%010x bytes",_phobjectmap[hdes][i]->size);
          else
             (void)snprintf(size_str,SSIZE,"unknown");

          (void)fprintf(stream,"    %04d: object \"%-32s\" [%-24s] at 0x%010x virtual [size %-16s]\n",
                                                                                                    i,
...
{    
     ...
     (void)fprintf(stream,"\n\n%s\n\n",info);
     (void)fprintf(stream,"    hdes %04d address table at 0x%010x virtual\n",hdes,offset);
...
{   int  i;
    ...
    {  (void)fprintf(stderr,"    heap %04d mapped into process memory map at 0x%010x virtual\n",hdes,offset);
       (void)fflush(stderr);
    }
    else
...
#ifdef PHEAP_DEBUG
(void)fprintf(stderr,"log: %04d pointer 0x%010x\n",i,(unsigned long int)_phfraghead[hdes][i].next);
(void)fflush(stderr);
#endif /* PHEAP_DEBUG */
