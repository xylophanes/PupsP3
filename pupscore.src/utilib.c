...
_PUBLIC      int pupsighold_cnt[MAX_SIGS] = { 1 };              // Counter for signal states
...
_PUBLIC char *version                     = (char *)NULL,       // Version of the code
             ...
             *appl_fifo_dir               = (char *)NULL,       // Nmae of default FIFO patchboard
             ...
             *args[255]                   = { (char *)NULL },   // Secondry argument vector
...
{   (void)fprintf(stderr,"\nGeneric PUPS/P3 options\n");  
    ...
    (void)fprintf(stderr,"[-etrap:FALSE]   [-version:FALSE] [-vtag] [-usage:FALSE] [-verbose:FALSE]\n");
...
_PUBLIC int pups_get_ip_info(const char *dev_name, char *ip_addr, char *node_name)
{
    ...
    (void)snprintf(ip_addr, SSIZE, (char *)inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr));
    (void)close(fd);
...
{   int  f_index,
    ...
    do {   (void)pups_fgets(line,1024,mtab_stream);
          ...
          {  int  j,
             ...
             {  char fshadow_name[SSIZE] = "";
                ...
                {  c_index = (int)(drand48()*6.0);
                   ...
                   (void)strlcat(fshadow_name,cntl_str,strlen(cntl_str));
                }
    ...
    {  if(strcmp(ftab[i].fname,name) == 0 && ftab[i].homeostatic > 0)
       {
          ...
          (void)strlcpy(ftab[i].hname,"none",4);
    ...
    {  if(ftab[i].fdes != (-1))
       {  struct stat buf;
          ...
          if(ftab[i].stream == (FILE *)NULL)
             ...
             (void)snprintf(fbuf,SSIZE,"(file handle 0x%010x)", (unsigned long int)ftab[i].stream);
          #ifdef ZLIB_SUPPORT
          else if(ftab[i].zstream != (gzFILE *)NULL)
             (void)snprintf(fbuf,SSIZE,"(file zhandle 0x%010x)",(unsigned long int)ftab[i].zstream);
          #endif /* ZLIB_SUPPORT */
          ...
          {  
             ...
             {  int f_index;
                ...
                (void)snprintf(fdid,SSIZE,"[peristent heap at 0x%010x virtual]",(unsigned long int)htable[f_index].addr);
             }
             else
             #endif /* PERSISTENT_HEAP_SUPPORT */
...
{   if(err_action == NONE)
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&errstr_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_unlock(&errstr_mutex);
...
{   int    i;
    ...
    _IMMORTAL char   *s2_was[N_STREXT_STRINGS];   /* Copy of current argument string */
    ...
    /*--------------------------------------*/
    /* Need to initialise when entered here */
    /*--------------------------------------*/
...
_PUBLIC int strand(const _BOOLEAN init, const size_t size, char *s)
...
{   size_t    i;
    ...
    /*----------------------------------------------------*/
    /* Intialise the random number generator if requested */
    /*----------------------------------------------------*/

    if(init == TRUE)
       (void)srand48((unsigned long int)appl_pid);
...
{   size_t size_1,
    ...
    {  pups_set_errno(OK);
       ...
       {  while(s_index < size_1 && f_cnt != c_cnt)
          ...
          (void)strlcpy(s2,(char *)&s1[s_index],SSIZE);
       }
...
{     int  j;
      ...
      {  FILE *ctail_alias_stream = (FILE *)NULL;
         ...
         do {   (void)fgets(next_alias,256,ctail_alias_stream);
...
{   _IMMORTAL char aggregate_str[SSIZE] = ""; 
    ...
    if(*argc > t_args)
       return((char *)((unsigned long int)INVALID_ARG));
             ...
             {  if(quoted == TRUE)
                {  next_token[c_index] = '\0';
                 ...
                 {  quoted = TRUE;
                    ...
                    {  (void)strlcpy(args[t_args],(char *)&next_token[1],SSIZE);
...
_PUBLIC void pups_std_init(_BOOLEAN open_source,  /* TRUE if GPL copyrighted open source  */
                           ...
                           char         *argv[])  /* Secondry argument vector             */
...
{   int  i;
    ...
    {  
       ...
       if(isatty(0) == 1)
          ...
          (void)strlcpy(appl_ttyname,"/dev/tty",8);
    ...
    (void)strlcpy(appl_password,"notset",6);
    ...
    {  appl_secure = TRUE;
       ...
       if(isatty(0) == 0)
       {  (void)fgets(appl_password,256,stdin);
          appl_secure = TRUE;
       ...
       {  
          ...
          do {    int verify_cnt = 0;
                  ...
                  do {   if(verify_cnt > 0)
                         ...
                         (void)snprintf(password_banner,SSIZE,"%s PSRP (protocol %4.2F) password: ",appl_host,PSRP_PROTOCOL_VERSION);
                         (void)strlcpy(tmp_pw_1,getpass(password_banner),SSIZE);
                         (void)snprintf(password_banner,SSIZE,"%s PSRP (protocol %4.2F) verify password:",appl_host,PSRP_PROTOCOL_VERSION);
...
(void)fflush(stderr);
    ...
    {  int i,
       ...
       {  pos = strlen(appl_bin_name) + 1;
          ...
          if(pos > 0)
             (void)strlcpy(up_appl_name,(char *)&appl_bin_name[pos+1],SSIZE);
          else
    ...
    #ifdef EXTENDED_SIGNAL_SET 
    (void)pups_sighandle(SIGPIPE,"exit_handler",(void *)exit_handler, (sigset_t *)&blocked_set);
    #endif /* EXTENDED_SIGNAL_SET */
...
_PUBLIC int pups_write(const des_t f_id, const _BYTE *buf, const psize_t n_bytes)
...
{   int written       = 0,
    ...
    do {    written = write(f_id,&bbuf[total_written],(n_bytes - total_written));
...
{   int ret,
    ...
    char     *cmd_list[256] = { NULL };   /* List of command arguments       */
    _BOOLEAN looper;
...
_PRIVATE int             lock_cnt      [PUPS_MAX_LOCKS]        = { 0 };
_PRIVATE char            lock_name_list[PUPS_MAX_LOCKS][SSIZE] = { "notset" };
_PRIVATE char            lock_type_list[PUPS_MAX_LOCKS][SSIZE] = { "notset" };
...
{   int i,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&lock_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    {  if(strcmp(lock_name_list[i],name) == 0)
       {
          ...
          #ifdef PTHREAD_SUPPORT
          (void)pupsthread_mutex_unlock(&lock_mutex);
          #endif /* PTHREAD_SUPPORT */
...
{   int i,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&pups_fork_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    {  if(chtab[i].pid == (-1))
       {  
          ...
          {  chtab[i].pid      = pid;
             ...
             #ifdef PTHREAD_SUPPORT
             (void)pupsthread_mutex_unlock(&pups_fork_mutex);
             #endif /* PTHREAD_SUPPORT */
          ...
          else
          {
             ...
             #ifdef PTHREAD_SUPPORT
             /*-----------------------------------------*/
             /* Child must release any mutexes it holds */
             /*-----------------------------------------*/
...
{   int i;
    ...
    {  char line[SSIZE]     = "",
       ...
       (void)strlcpy(vendor_domain,(char *)&vendor_mail_address[i+1],SSIZE);   
       (void)snprintf(ping_cmd,SSIZE,"csh -c \"ping -c 1 %s |& cat\"",vendor_domain);

       if((pstream = popen(ping_cmd,"r")) == (FILE *)NULL)
          return;

       (void)fgets(line,256,pstream);
       (void)fgets(line,256,pstream);
       (void)pclose(pstream);
...
{   FILE *info_stream = (FILE *)NULL,
    ...
    (void)snprintf(tty_info_file,SSIZE,"/tmp/info.tmp.%d",getpid());
    info_stream = pups_fopen(tty_info_file,"r",LIVE);
    (void)fscanf(info_stream,"%s",tty_dev_name);
...
/*-------------------------------------------------------------------------
    ...
    Note that we had to change this from _PRIVATE to _PUBLIC because
    CKPT cannot deal with static pointers which have memory dynamically
    allocated to them (28/10/97) ...
...
{   int  i;
    ...
    {   if((void *)pups_entrance_f[i] == (void *)NULL)
        {  pups_entrance_f[i] = entrance_f; 
           ...
           {  strdate(date);
              (void)fprintf(stderr,
                      "%s %s (%d@%s:%s): PUPS entrance function %s (0x%010x) registered at slot %d\n",
                                                                                                date,
                                                                                           appl_name,
                                                                                            getpid(),
                                                                                           appl_host,
    ...
    if(appl_verbose == TRUE)
    {  strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): PUPS entrance function %s (at 0x%010x virtual) registered at slot %d\n",
                                                                                                                  date,
                                                                                                             appl_name,
                                                                                                              getpid(),
                                                                                                             appl_host,
                                                                          ...
                                                                          pups_entrance_f_name[n_entrance_funcs_alloc], 
                                                                               pups_entrance_f[n_entrance_funcs_alloc],
...
{   int i;
    ...
    {  if((void *)pups_entrance_f[i] == func)
       {  if(appl_verbose == TRUE) 
          {  strdate(date);
             (void)fprintf(stderr, "%s %s (%d@%s): Entrance function %s (at 0x%010x virtual) at slot %d deregistered\n",
                                                                                                                   date,
                                                                                                              appl_name,
                                                                                                               getpid(),
                                                                                                              appl_host,
...
{   int i;
    ...
    {  (void)fprintf(stream,"\n\n    Entrance functions registered\n");
       ...
       for(i=0; i<n_entrance_funcs_alloc; ++i)
       {  if((void *)pups_entrance_f[i] != (void *)NULL)
          {  (void)fprintf(stream,"    %04d: \"%-32s\" (at 0x%010x virtual)\n",i,
                                                         pups_entrance_f_name[i],
...
{   int      i;
    ...
    {  if((void *)pups_entrance_f[i] != (void *)NULL)
       {  (void)strdate(date);
          if(appl_verbose == TRUE)
          {  (void)fprintf(stderr,"%s %s (%d@%s:%s): executing entrance function %s (slot %d at 0x%010x virtual)\n",
                                                                       date,appl_name,appl_pid,appl_host,appl_owner,
                                                                                            pups_entrance_f_name[i],
                                                                                                                  i,
                                                                              (unsigned long int)pups_entrance_f[i]);
...
{   int  i;
    ...
    {   if((void *)pups_exit_f[i] == (void *)NULL)
        {  pups_exit_f[i] = exit_f; 
           ...
           {  strdate(date);
              (void)fprintf(stderr,
                      "%s %s (%d@%s:%s): PUPS exit function %s (0x%010x) registered at slot %d\n",
                                                                                             date,
                                                                                        appl_name,
                                                                                         getpid(),
                                                                                        appl_host,
    ...
    {  strdate(date);
       (void)fprintf(stderr,
               "%s %s (%d@%s:%s): PUPS exit function %s (at 0x%010x virtual) registered at slot %d\n",
                                                                                                 date,
                                                                                            appl_name,
                                                                                             getpid(),
                                                                                            appl_host,
                                                                 ...
                                                                 pups_exit_f_name[n_exit_funcs_alloc], 
                                                                      pups_exit_f[n_exit_funcs_alloc],
                                                                                   n_exit_funcs_alloc);
       (void)fflush(stderr);
    }
...
{   int i;
    ...
    {  if((void *)pups_exit_f[i] == func)
       {  if(appl_verbose == TRUE) 
          {  strdate(date);
             (void)fprintf(stderr,"%s %s (%d@%s): Exit function %s (at 0x%010x virtual) at slot %d deregistered\n",
                                                                                                              date,
                                                                                                         appl_name,
                                                                                                          getpid(),
                                                                                                         appl_host,
...
{   int i;
    ...
    {   if((void *)pups_abort_f[i] == (void *)NULL)
        {  pups_abort_f[i] = abort_f;
           ...
           if(appl_verbose == TRUE)
           {  (void)strdate(date);
              (void)fprintf(stderr,"%s %s (%d@%s:%s): PUPS abortfunction %s (0x%010x) registered at slot %d\n",
                                                                  date,appl_name,appl_pid,appl_host,appl_owner,
                                                                                          pups_abort_f_name[i],
                                                                                               pups_abort_f[i],
                                                                                                             i);
    ...
    if(arg_str != (char *)NULL)
      (void)strlcpy(pups_exit_arg[n_abort_funcs_alloc],arg_str,SSIZE);

    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): PUPS abort function %s (at 0x%010x virtual) registered at slot %d\n",
                                                                       date,appl_name,appl_pid,appl_host,appl_owner,
                                                                             pups_abort_f_name[n_abort_funcs_alloc],
                                                                                  pups_abort_f[n_abort_funcs_alloc],
                                                                                                n_abort_funcs_alloc);
       (void)fflush(stderr);
    }
...
{   int i;
    ...
    {  if((void *)pups_abort_f[i] == func)
       {  if(appl_verbose == TRUE)
          {  (void)strdate(date);
             (void)fprintf(stderr,"%s %s (%d@%s:%s): PUPS abort function %s (at 0x%010x virtual) at slot %d deregistered\n",
                                                                               date,appl_name,appl_pid,appl_host,appl_owner,
                                                                                                       pups_abort_f_name[i],
                                                                                                            pups_abort_f[i],
                                                                                                                          i);
...
{   int  i,
    ...
    {  if(appl_verbose == TRUE)
       {  pthread_t tid;
          ...
          (void)fprintf(stderr,"%s %s (%d@%s:%s): thead 0x%010x (%s) exiting\n",
                                   date,appl_name,appl_pid,appl_host,appl_owner,
    ...
    {  if((void *)pups_exit_f[i] != (void *)NULL)
       {  if(appl_verbose == TRUE)
          {  (void)strdate(date);
             (void)fprintf(stderr,"%s %s (%d@%s:%s): executing exit function %s (slot %d at 0x%010x virtual)\n",
                                                                   date,appl_name,appl_pid,appl_host,appl_owner,
                                                                                            pups_exit_f_name[i],
                                                                                                              i,
                                                                              (unsigned long int)pups_exit_f[i]);
    ...
    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"\n%s %s (%d@%s:%s): %d application specific exit functions to execute\n\n",
...
{   int  pid;
    ...
    {  if(strccpy(exec_pathname,pups_search_path("PATH",ben_name)) == (char *)NULL)
       ...
       (void)strlcpy(exec_pathname,ben_name,strlen(ben_name));
...
{   sigset_t  set;
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&sigtab_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    {  ++pupsighold_cnt[signum];
       ...
       #ifdef PTHREAD_SUPPORT
       (void)pupsthread_mutex_unlock(&sigtab_mutex);
       #endif /* PTHREAD_SUPPORT */
    ...
    {  (void)sigpending(&set);
       ...
       {  
          ...
          /*-----------------------------------------------------------*/
          /* Unmask only the signal released signal (which is pending) */
          /* and await its delivery  and SIGABRT as we may need to     */
          /* service a server interrupt in this state                  */
          /* We need SIGALRM to service homeostasis functions          */
          /*-----------------------------------------------------------*/
    ...
    /*-----------------------------------------------------------*/
    /* Check to see if target signal of this release is pending  */
    /* we must block the signum to prevent it being taken before */
    /* sigsuspend is executed                                    */
    /*-----------------------------------------------------------*/

    if(in_pupshold == FALSE)
    {  (void)sigaddset(&old_set,signum);
       (void)pups_sigprocmask(SIG_BLOCK,&old_set,(sigset_t *)NULL);
    }
...
{   sigset_t set;
    ...
    (void)sigfillset(&set);
    (void)pups_sigprocmask(SIG_BLOCK,&set,&old_set);
    if(pupsighold_cnt[signum] < 0) /* MAO was <= */
    {  char errstr[SSIZE] = "";
...
{   int i,
    ...
    {  if(sigtab[i].haddr == (unsigned long int)SIG_IGN)
       ...
       {  if(sigtab[i].haddr != (unsigned long int)SIG_DFL)
          {  (void)fprintf(stream,
                     "    %-16s (%02d): re-entrant handler at 0x%010x virtual",
                                                signame[i],i,sigtab[i].haddr);
...
{   int      i;
    ...
    (void)fprintf(stream,"    Handler \"%-32s\" installed at 0x%010x virtual\n",sigtab[signum].hname,
    ...
    {  if(sigismember(&sigtab[signum].sa_mask,i+1) == 1)
       {  (void)fprintf(stream,"    Signal %-16s [%04d] is blocked while handler for %-16s [%04d] is running\n",
                                                                          signame[i],i+1,signame[signum],signum);
          (void)fflush(stream);
...
{   time_t tval;
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&date_mutex);
    #endif /* PHTREAD_SUPPORT */
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_unlock(&date_mutex);
    #endif /* PHTREAD_SUPPORT */
...
free_timer:
    ...
    {  (void)strdate(date);
       if(mode == VT_CONTINUOUS)
          (void)fprintf(stderr,"%s %s (%d@%s:%s): PUPS virtual timer %d (%s) continuous timer, priority %d (interval %4.2F secs)\n",
                                                                                                                              date,
                                                                                                                         appl_name,
                                                                                                                          appl_pid,
                                                                                                                         appl_host,
          ...
          (void)fprintf(stderr,"%s %s (%d@%s:%s): PUPS virtual timer %d (%s) one shot timer, priority %d (interval %4.2F secs)\n",
                                                                                                                             date,
                                                                                                                        appl_name,
                                                                                                                         appl_pid,
                                                                                                                        appl_host,
...
{   int i;
    ...
    {  (void)fprintf(stream,"\n\n    Virtual timers active\n");
       ...
       (void)fflush(stream);
        ...
        (void)fprintf(stream,"    Virtual timer quantum is %6.4F seconds\n\n",(FTYPE)vitimer_quantum*1.e-7);
        (void)fflush(stream);
        ...
        {  if(vttab[i].priority > 0)
           {  if(vttab[i].mode == VT_CONTINUOUS)
                 (void)fprintf(stream,
                               "    %04d: \"%-32s\" priority %02d, continous mode, interval time %6.4F secs, prescaler %04d, (handler at 0x%010x virtual)\n",i,
                                                                                                                                                 vttab[i].name,
                                                                                                                                             vttab[i].priority,
                ...
                (void)fprintf(stream,
                              "    %04d: \"%-32s\" priority %02d, oneshot mode, interval time %6.4F secs, prescaler %04d, (handler at 0x%010x virtual)\n",i,
                                                                                                                                              vttab[i].name,
                                                                                                                                          vttab[i].priority,
			                                                                                              (FTYPE)vttab[i].interval_time / 100.0,
                                                                                                                                         vttab[i].prescaler,
...
{   int i;
    ...
    /* which is used to implement pups_usleep() in a manner which does */
...
{   struct   itimerval itimer;
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&malarm_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_unlock(&malarm_mutex);
    #endif /* PTHREAD_SUPPORT */
...
{   int  f_index;
    ...
    {  (void)fstat(ftab[f_index].fdes,&buf);
       ...
       {  if(appl_verbose == TRUE)
          {  strdate(date);
             (void)fprintf(stderr,"%s %s (%d@%s:%s): %s \"%s\" protection mode changed (from 0x%d to 0x%d) -- restoring\n",date,
                                                                                                                      appl_name,
                                                                                                                       appl_pid,
                                                                                                                      appl_host,
                                                                                                                     appl_owner,
    ...
    (void)gethostname(current_hostname,256);
...
{   
    ...
    (void)pupsthread_mutex_lock(&matab_mutex);
...
{   int  i,
    ...
    {  if(matab.object[i] == (matab_object_type *)NULL)
       {  ptr                   = pups_malloc(size); 
          ...
          #ifdef PTHREAD_SUPPPORT
          (void)pupsthread_mutex_unlock(&matab_mutex);
          #endif /* PTHREAD_SUPPPORT */
...
{   int i;
    ...
    for(i=0; i<matab.allocated; ++i)
    {  if(matab.object[i] != (matab_object_type *)NULL && matab.object[i]->ptr == ptr)
       {  (void)fprintf(stream,"    Object %-32s (type %-32s) size 0x%010x (at 0x%010x virtual)\n",matab.object[i]->name,
                                                                                                   matab.object[i]->type,
                                                                                                   matab.object[i]->size,
...
{   int i,
    ...
    {  (void)fprintf(stream,"    %04d: Object %-32s (type %-32s) size 0x%010x (at 0x%010x virtual)\n",i,
    ...
    if(t_cnt > 1)
       (void)fprintf(stream,"\n\n    %04d tracked objects [in 0x%010x bytes of heap] (%04d slots free)\n\n",t_cnt,heap_used,matab.allocated - t_cnt);
    else if(t_cnt == 1)
...
(void)fflush(stderr);
          ...
          (void)snprintf(pups_password_banner,SSIZE,"PUPSP3 %s [%s] login: ",password_banner,appl_host);
...
{   int  next_pid;
    ...
    (void)fgets(line,256,procstream);
    (void)fgets(line,256,procstream);
...
/*--------------------------------------------------------------------------------------
    Wait for a file to load completely ...
--------------------------------------------------------------------------------------*/
...
_PUBLIC void pups_fdes_set_stable(void)
{   fdes_stable = TRUE;
}
...
_PUBLIC void pups_fdes_reset_stable(void)
{   fdes_stable = FALSE;
}


_PUBLIC void pups_wait_until_file_loaded(const int fdes)

{   int               ret;
    struct stat       buf;
    unsigned long int delay;
    off_t             current_size = 0;
    _BOOLEAN          net_file     = FALSE;


    /*---------------------------------------------------*/
    /* If file is stable -- we don't need to do anything */
    /*---------------------------------------------------*/
    ...
    /*--------------------------------------------------*/
    /* Check to see if file is on a network file system */
    /*--------------------------------------------------*/

    ret = pups_is_on_fnfs(fdes);
    if(ret < 0)
       return;
    else if(ret  == TRUE)
    {  delay    = 100;
    ...
    while(buf.st_size > current_size || current_size == 0)
...
/*---------------------------------------------------------------------------------------
    Make sure that pups_usleep() and alarm() do not interact with one another ...
---------------------------------------------------------------------------------------*/
...
{   int i;
    ...
    /*----------------------------------------------*/ 
    /* Hoemostasis is stopped while we are sleeping */
    /* so we should only call PUPS usleep for small */
    /* amounts of time. Any time period longer than */
    /* a second is treated as an error              */
    /*----------------------------------------------*/ 
...
{   DIR           *dirp      = (DIR *)NULL;
    ...
    {   int  pid;
        ...
        {  int i;
           ...
           (void)fgets(cmd_line,256,stream);
           (void)fclose(stream);
        }
    }
...
{   unsigned short irc1(unsigned short int crc, _BYTE onech);
    ...
    _IMMORTAL unsigned short icrctb[256] = { 0 },
                             init        = 0;

    _IMMORTAL _BYTE rchr[SSIZE] = "";
...
_PRIVATE unsigned long int crc64_lookup_table[256]  = { 0L };

_PRIVATE void generate_crc64_lookup_table(void)
{   int i,
        j;

    pthread_mutex_t crc64_init_mutex;
...
_PRIVATE int    lock_fdes[PUPS_MAX_FLOCKS]         = { (-1) };
_PRIVATE int    lock_type[PUPS_MAX_FLOCKS]         = { (-1) };
_PRIVATE off_t  lock_start[PUPS_MAX_FLOCKS]        = {  0   };
_PRIVATE off_t  lock_size[PUPS_MAX_FLOCKS]         = {  0   };
...
_PRIVATE char   lock_name[PUPS_MAX_FLOCKS][256] = { "" };
...
{   int i,
    ...
    {  if(lock_fdes[i] != (-1))
       {  if(lock_type[i] == F_RDLCK)
             (void)fprintf(stream,"    %04d: (slot %04d): %-32s [READLOCK  starting 0x%010x, length 0x%010x]",flock_cnt,i,lock_name[i],lock_start[i],lock_size[i]);  
          else if(lock_type[i] == F_WRLCK)
             (void)fprintf(stream,"    %04d: (slot %04d): %-32s [WRITELOCK starting 0x%010x, length 0x%010x]]",flock_cnt,i,lock_name[i],lock_start[i],lock_size[i]);  
          (void)fflush(stream);
...
{   char strdum[SSIZE]     = "",
    ...
    do {    (void)fgets(line,256,pstream);
...
{   FILE *stream           = (FILE *)NULL;
    ...
    (void)fgets(versionstr,256,stream);
    (void)fclose(stream);
...
_PUBLIC int pups_popen(const char *shell, const char *cmd, const int rw_flags, int *child_pid)
{   int fildes[2];
...
FILE *pups_fpopen(const char *shell, const char *cmd, const char *rw_flags, int *child_pid)
...
int xpclose(const int pdes, const int pid)
...
{    char line[SSIZE]        = "",
     ...
     (void)snprintf(pstream_cmd,SSIZE,"mpstat | tail -1 | awk '{print $12}'");
...
_PUBLIC _BOOLEAN pups_is_in_container(void)

{   char str[SSIZE] = "",
         cmd[SSIZE] = "ps aux | grep systemd | head -1";
...
{   unsigned long int file_size,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&copy_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    {
       #ifdef PTHREAD_SUPPORT
       (void)pupsthread_mutex_unlock(&copy_mutex);
       #endif /* PTHREAD_SUPPORT */
