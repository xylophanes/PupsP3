/*--------------------------------------------------------------------------------
    Purpose: Multiplication function for shell scripts
...
{   int ret = 0;
    ...
    if(argc != 3                                          ||
       ...
       strncmp(argv[1],"--help",strlen(argv[1])) == 0      )
    {  (void)fprintf(stderr,"\nadd version %s, (C) Tumbling Dice 2000-2022 (built %s %s)\n\n",FMUL_VERSION,__TIME__,__DATE__);
       ...
       (void)fprintf(stderr,"\nusage: fmul <arg1> <arg2> [>& [<arg1> * <arg2>]\n\n");
       (void)fflush(stderr);
