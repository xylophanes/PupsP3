/*-------------------------------------------------------------
    Purpose: floating point subtract function for shell scripts
...
{   int32_t ret = 0;
    ...
    if(argc != 3                                          ||
       ...
       strncmp(argv[1],"--help",strlen(argv[1])) == 0      )
    {  (void)fprintf(stderr,"\nfsub version %s, (C) Tumbling Dice 2000-2024 (gcc %s: built %s %s)\n\n",FSUB_VERSION,__VERSION__,__TIME__,__DATE__);
