...
static int term_handler(const int signum)
...
static int wait_for_termination(const int signum)
{
...
{  int           softdog_pid         = (-1);
   ...
   (void)fprintf(stderr,"\nsoftdog tester version %s, (C) Tumbling Dice, 2023 (built %s %s)\n\n",SDTEST_VERSION,__TIME__,__DATE__);
   ...
   /*------------------------------------*/
   /* Keep feeding the specified softdog */
