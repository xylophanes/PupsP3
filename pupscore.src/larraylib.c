...
{   (void)fprintf(stderr,"lib larraylib %s: [ANSI C]\n",LARRAY_VERSION);
    ...
    if(level > 1)
    {  (void)fprintf(stderr,"(C) 2013-2019 Tumbling Dice\n");
...
{   int i,
    ...
    {  (void)strdate(date);
       (void)fprintf(stream,"%s %s (%d@%s:%s):  List array compression factor is %4.2F%%\n",
...
{   int i,
    ...
    else
    {
       ...
       {  (void)strdate(date);
          (void)fprintf(stream,"%s %s (%d@%s:%s)  List vector compression factor is %4.2F%%\n",
...
{   int i,
    ...
    {  if(matrix_squeeze_percent != (FTYPE *)NULL)
       ...
       (void)strdate(date);
       (void)fprintf(stream,"%s %s (%d@%s:%s)  List matrix deflated by %4.2F %%\n",
                                      date,appl_name,appl_pid,appl_host,appl_owner,
