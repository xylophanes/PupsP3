...
{   int ret,
    ...
    ret = execlp("tcsh","tcsh","-c",exec_command,(char *)NULL);
...
{   int i,
    ...
    if(argc < 3 || argc > 4)
    {  (void)fprintf(stderr,"\nfarm version %s, (C) Tumbling Dice 2002-2019 (built %s %s)\n\n",FARM_VERSION,__TIME__,__DATE__); 
    ...
    {  if(strcmp(argv[1],"-verbose") != 0)
       {  (void)fprintf(stderr,"\nfarm version %s, (C) Tumbling Dice 2019 (built %s %s)\n\n",FARM_VERSION,__TIME__,__DATE__);
          (void)fprintf(stderr,"FARM is free software, covered by the GNU General Public License, and you are\n");
          (void)fprintf(stderr,"welcome to change it and/or distribute copies of it under certain conditions.\n");
          (void)fprintf(stderr,"See the GpL and LGPL licences at www.gnu.org for further details\n");
          (void)fprintf(stderr,"FARM comes with ABSOLUTELY NO WARRANTY\n\n");
