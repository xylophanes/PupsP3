...
{   (void)fprintf(stdout,"int app psrp %s: [ANSI C]\n",PSRP_VERSION);
    ...
    if(level > 1)
    {  (void)fprintf(stdout,"(C) 1994-2018 Tumbling Dice\n");
       (void)fprintf(stdout,"Author: M.A. O'Neill\n");
       (void)fprintf(stdout,"PSRP (protocol %4.2F) communication shell (built %s %s)\n",PSRP_PROTOCOL_VERSION,__TIME__,__DATE__);
       (void)fflush(stdout);
    }
...
{   (void)fprintf(stdout,"[-startmsg:FALSE]\n"); 
    ...
    (void)fprintf(stdout, "SIGPSRP, SIGINIT, SIGCHAN, SIGCLIENT: PSRP status request signals (protocol %4.2F)\n",PSRP_PROTOCOL_VERSION);
...
_PRIVATE char mstack_f_name[MAX_PSRP_MACRO_FILES][SSIZE] = { "" };
...
// Handler for SIGPSRP
_PROTOTYPE _PRIVATE int client_handler(int);

// Handler for SIGCHAN
_PROTOTYPE _PRIVATE int chan_client_handler(int);
...
_PROTOTYPE _PRIVATE void psrp_close_server(_BOOLEAN);
...
// Propagate SIGINT to PSRP server process
_PROTOTYPE _PRIVATE int psrp_int_handler(int);
...
_PROTOTYPE _PRIVATE void psrp_grab_channel(void);
...
/*-----------------------------------------------------------------------------*/
/* Software I.D. tag (used if CKPT support is enabled to discard stale dynamic */
/* checkpoint files)                                                           */
/*-----------------------------------------------------------------------------*/
...
{   char psrp_request[SSIZE]    = "",
    ...
    {  (void)fprintf(stderr,"\n    PSRP client version %s, [build %6d]\n",PSRP_VERSION,appl_vtag);
       (void)fprintf(stderr,"    (C) M.A. O'Neill, Tumbling Dice 2018\n\n");
       (void)fflush(stderr);
    }
...
(void)fflush(stderr);
    ...
    (void)pups_sighandle(SIGCLIENT,"client_handler",     (void *)client_handler,      &client_set);
...
(void)fflush(stderr);
    ...
    if(psrp_log == TRUE)
    {  (void)fprintf(stdout,"\npsrp(protocol %4.2F): closing communication channel (%d transactions)\n\n",
...
{   char promptc,
    ...
    do {   int ret,
           ...
           do {
                  ...
                  if((restore_signum = sigsetjmp(command_loop_top,1)) > 0) // MAO Was 1 
                  {  
...
prompt:           if(server_pid != 0)
                  ...
                  {  char prompt_str[SSIZE] = "",
                     ...
                     {  char history_line[SSIZE] = "";
                        ...
                        if(strexccpy(request_line,history_line,"\t") > 0)
...
(void)fflush(stderr);
            ...
            {  int  args;
               ...
               else if(strcmp(tmpstr,"default") == 0 || strcmp(tmpstr,"less") == 0)
               {  do_datasink = TRUE;
               ...
               else if(strcmp(tmpstr,"more") == 0 || strcmp(tmpstr,"less") == 0)
               {  do_datasink = TRUE;
	    ...
	    {  server_verbose        = TRUE;
            ...
            {  if(pel_appl_verbose == TRUE)
               {  char machtype[SSIZE] = "",
                  ...
                  (void)fprintf(stdout,"\nPSRP interaction client %s (PSRP protocol %4.2F, PML version %4.2F)\n",
            ...
            if(strncmp(request,"test",4) == 0)
            {  (void)fprintf(stdout,"TEST\n");
               (void)fflush(stdout);
              ...
              if(ret_state < 0)
              {
                 ...
                 if(psrp_migrate_client_to_server_host() == 0)
                 {
                    ...
                    {  if(ret == PUPS_TERMINATED)
                       {  (void)fprintf(stdout,"\ntarget process %s (%d@%s) has been terminated\n\n",
                                                                    psrp_server,server_pid,psrp_host);
                          (void)fflush(stdout);
                       }
                       else if(ret == PUPS_STOPPED && psrp_hard_link == FALSE)
                       {  (void)fprintf(stdout,"\ntarget process %s (%d@%s) has been stopped\n\n",
                                                                 psrp_server,server_pid,psrp_host);
                       (void)fflush(stdout);
                       }
                    }
...
(void)fflush(stderr);
              ...
              do {
                     ...
                     {  (void)pups_clearvitimer("server_alive_handler");
                        if(pel_appl_verbose == TRUE)
                        {  (void)fprintf(stdout,"\nconnection closed (server termination by client)\n\n");
                           (void)fflush(stdout);
                        }
...
_PRIVATE int tstp_handler(int signum)
...
_PRIVATE int client_handler(int signum)
...
(void)fflush(stdout);
    ...
    if(psrp_have_trailfile() == TRUE)
    {
       ...
       {  (void)read(0,pending_request,SSIZE);
       ...
       {  (void)strdate(date);
          (void)fprintf(stdout,"\nserver segment (%d %s (%d@%s):%s):  reconnection failed\n",
...
_PRIVATE int chan_client_handler(int sig)
...
{   int trys = 0;
    ...
    while((client_in  = pups_fopen((char *)channel_name_in, "r+",DEAD)) == (FILE *)NULL && trys < max_trys);
    {     (void)pups_usleep(100);
          ++trys;
    }
    ...
    trys = 0;
    while((client_out = pups_fopen((char *)channel_name_out,"r+",DEAD)) == (FILE *)NULL && trys < max_trys);
    {     pups_usleep(100);
          ++trys;
    }
    ...
    {  if(client_in == (FILE *)NULL || client_out == (FILE *)NULL)
       {  if(pel_appl_verbose == TRUE)
          {  (void)strdate(date);
             (void)fprintf(stdout,"%s %s (%d@%s:%s) broken PSRP channel\n",
                                                                      date,
                                                               psrp_server,
                                                                server_pid,
                                                                 psrp_host,
    ...
    {  if(pel_appl_verbose == TRUE)
       {  (void)strdate(date);
          (void)fprintf(stdout,"%s %s (%d@%s:%s): stopped\n",
                                                        date,
                                                 psrp_server,
                                                  server_pid,
                                                   psrp_host,
    ...
    if(pel_appl_verbose == TRUE)
    {  (void)printf("\npsrp(protocol %4.2F): opening PSRP communication channel to %s (%d@%s)\n\n",
...
(void)fflush(stderr);
    ...
    /*------------------------------------*/
    /* Release channel access lock        */
    /* unless we have been specifically   */
    /* instructed to keep the file locked */
    /*------------------------------------*/
...
_PRIVATE void psrp_close_server(_BOOLEAN tell_server)
...
{   int i,
    ...
    ftruncate(fileno(client_in), 0);
    ftruncate(fileno(client_out),0);
...
{    FILE *pstream = (FILE *)NULL;
     ...
     (void)fprintf(pstream,"    ===========================\n\n");
     (void)fprintf(pstream,"    open <PSRP server>[@<host>]  : open PSRP server process <PSRP server> [on <host>]\n");
     (void)fprintf(pstream,"    close                        : close PSRP server process\n");
     (void)fprintf(pstream,"    trys <N trys>                : set number of attempts to open PSRP server\n");
     ...
     (void)fprintf(pstream,"    sink  [<datasink cmd>] [bg]  : redirect PSRP client output to datasink command [in background]\n");
     (void)fprintf(pstream,"    clean <PSRP channel dir>     : clean stale PSRP channels from PSRP channel (patchboard) directory\n");
     (void)fprintf(pstream,"    quit | exit | bye            : terminate psrp client\n");
     (void)fprintf(pstream,"    chanstat [-all] <directory>  : show active PSRP channels in (patchboard) <directory>\n");
...
_PRIVATE int psrp_int_handler(int sig)
...
{   int  i,
    ...
    {  if(request[i] == ':' && request[i+1] != ' ' && isdigit(request[i+1]) == 0)
       ...
       if(request[i] == '"' && strin(request,"!") == FALSE)
       {  if(glob == TRUE)
             glob = FALSE;
          else
             glob = TRUE;
    ...
    {  i = 0;
       ...
       {  
          ...
          char psrp_client[SSIZE]                 = "",
               psrp_parameters[SSIZE]             = "",
               host_list[MAX_PSRP_SERVERS][SSIZE] = { "" }; 
...
_PRIVATE void psrp_grab_channel(void) 
...
{   int i,
    ...
    char     tmp_str[SSIZE]                       = "",
             next_item[SSIZE]                     = "",
             argument_list[MAX_MACRO_ARGS][SSIZE] = { "" };
...
{   int i,
    ...
    char clash_tags[MAX_CLASH_TAGS][SSIZE] = { "" };
          ...
          {  char   state[SSIZE] = "";
             ...
             {  (void)fprintf(stdout,"%-15s\t\t%-15s\t\t%-15s\t\t%d",application,pwent->pw_name,state,pid);
                ...
                else
                {
                   ...
                   (void)readlink(procpidpath,binname,SSIZE);
...
{   char strdum[SSIZE]   = "",
    ...
    if(geteuid() != 0)
    {  (void)fprintf(stdout,"Cannot change session owner unless client (psrp) is installed set UID root\n");
       (void)fflush(stdout);
    ...
    (void)printf("\n%s PSRP (protocol %4.2F) changing PSRP session owner (from %s to %s)\n",
...
{   char password_banner[SSIZE] = "",
    ...
    do {    int verify_cnt = 0;
            ...
            {  do {   if(verify_cnt > 0)
                      ...
                      (void)snprintf(password_banner,SSIZE,"%s PSRP (protocol %4.2F) new password: ",appl_host,PSRP_PROTOCOL_VERSION);
                      (void)strlcpy(tmp_pw_1,getpass(password_banner),SSIZE);
                      ...
                      (void)snprintf(password_banner,SSIZE,"%s PSRP (protocol %4.2F) verify new password:",appl_host,PSRP_PROTOCOL_VERSION);
                      (void)strlcpy(tmp_pw_2,getpass(password_banner),SSIZE);
...
{   _IMMORTAL _BOOLEAN update_cnt = 0;
    ...
    (void)printf("\n%s PSRP (protocol %4.2F) setting %sauthentication token for PSRP services\n",
...
_PRIVATE int winch_handler(int signum)
...
(void)fflush(stderr);
    ...
    if(interactive_mode == FALSE || psrp_have_trailfile() == FALSE /*|| strcmp(trailfile_name,"") == 0*/) // MAO added trailfile name test
    {
...
{   FILE *stream          = (FILE *)NULL,
    ...
    do {   if(clist_alloc == 0)
           ...
           clist[clist_size] = (char *)pups_malloc(SSIZE*sizeof(char));
