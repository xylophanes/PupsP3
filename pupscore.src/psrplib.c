...
_PROTOTYPE _PRIVATE int psrp_builtin_set_nodetach(int, char *[]);

// Set PSRP (vitimer) quantum
_PROTOTYPE _PRIVATE int psrp_builtin_set_vitimer_quantum(int, char *[]);

// Set (secure) server authentication token
_PROTOTYPE _PRIVATE int psrp_builtin_set_secure(int, char *[]);
...
_PROTOTYPE _PRIVATE int psrp_builtin_set_mime_type(int, char *[]);
#endif /* MAIL_SUPPORT */


// Display (fast) caches mapped into the address space of caller
_PROTOTYPE _PRIVATE int psrp_builtin_show_caches(int, char *[]);

// Parse a PSRP request (noting which interface originated it)
_PROTOTYPE _PRIVATE int psrp_parse_request(char *, int);

// Show host information
_PROTOTYPE _PRIVATE int psrp_builtin_show_hinfo(int, char *[]);


#ifdef BUBBLE_MEMORY_SUPPORT 
// Set (J)malloc options
_PROTOTYPE _PRIVATE int psrp_builtin_set_mbubble_utilisation_threshold(int, char *[]);

// Display (J)malloc statistics
_PROTOTYPE _PRIVATE int psrp_builtin_show_malloc_stats(int, char *[]);
#endif /* BUBBLE_MEMORY_SUPPORT */

#ifdef CRIU_SUPPORT
// Enable/disable (Criu) state saving
_PROTOTYPE _PRIVATE int psrp_builtin_ssave(int, char *[]);
#endif /* CRIU_SUPPORT */


// Process exits if its effective parent is terminated
_PROTOTYPE _PRIVATE int psrp_builtin_set_pexit(int, char *[]);

// Process does not exit if its effective parent is terminated
_PROTOTYPE _PRIVATE int psrp_builtin_set_unpexit(int, char *[]);

// Set (effective) parent
_PROTOTYPE _PRIVATE int psrp_builtin_set_parent(int, char *[]);

// Set system context to rooted (system context cannot migrate)
_PROTOTYPE _PRIVATE int psrp_builtin_set_rooted(int, char *[]);

// Set system context to unrooted (system context can migrate)
_PROTOTYPE _PRIVATE int psrp_builtin_set_unrooted(int, char *[]);
  
// Set current working directory for PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_set_cwd(int, char *[]);
   
// Set number of times operation is retryed (before aborting)
_PROTOTYPE _PRIVATE int psrp_builtin_set_trys(int, char *[]);

// Create trailfile and associated destructor lyosome
_PROTOTYPE _PRIVATE void psrp_create_trailfile(char *, char *, char *, char *);

// Protect a file
_PROTOTYPE _PRIVATE int psrp_builtin_file_live(int, char *argv[]);

// Unprotect a file
_PROTOTYPE _PRIVATE int psrp_builtin_file_dead(int, char *argv[]);

// Show resource usage for this server
_PROTOTYPE _PRIVATE int psrp_builtin_show_rusage(int, char *[]);

// Set resource usage for this server
_PROTOTYPE _PRIVATE int psrp_builtin_set_rlimit(int, char *[]);

// Builtin to display crontab contents
_PROTOTYPE _PRIVATE int psrp_builtin_show_crontab(int, char *[]);

// Builtin to schedule a crontab operation
_PROTOTYPE _PRIVATE int psrp_builtin_crontab_schedule(int, char *[]);

// Builtin to unschedule a crontab operation
_PROTOTYPE _PRIVATE int psrp_builtin_crontab_unschedule(int, char *[]);
...
_PROTOTYPE _PRIVATE int psrp_builtin_alias(int, char *[]);

// Builtin to show process status
_PROTOTYPE _PRIVATE int psrp_builtin_show_procstatus(int, char *[]);

// Show the (current) clients attached to PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_show_clients(int, char *[]);

// Create a new proces instance of PSRP server process
_PROTOTYPE _PRIVATE int psrp_builtin_new_instance(int, char *[]);
...
_PROTOTYPE _PRIVATE int psrp_builtin_unalias(int, char *[]);

// Show aliases for objects bound to this PRSP server
_PROTOTYPE _PRIVATE int psrp_builtin_showaliases(int, char *[]);
...
_PROTOTYPE _PRIVATE int psrp_builtin_appl_verbose(int, char *[]);

// Delete object from PSRP dispatch table
_PROTOTYPE _PRIVATE int psrp_builtin_transactions(int, char *[]);

// Delete object from PSRP dispatch table
_PROTOTYPE _PRIVATE int psrp_builtin_detach_object(int, char *[]);

// Show state of PSRP bindings (to client)
_PROTOTYPE _PRIVATE int psrp_builtin_show_psrp_state(int, char *[]);

// Show bindings of all PSRP objects
_PROTOTYPE _PRIVATE int psrp_builtin_show_psrp_bind_type(int, char *[]);

// Terminate PSRP server process
_PROTOTYPE _PRIVATE int psrp_builtin_terminate_process(int, char *[]);


#ifdef SSH_SUPPORT
// Set ssh compression
_PROTOTYPE _PRIVATE int psrp_builtin_ssh_compress(int, char *[]);

// Set remote ssh port
_PROTOTYPE _PRIVATE int psrp_builtin_ssh_port(int, char *[]);
#endif /* SSH_SUPPORT */

// Display help for psrp_handler
_PROTOTYPE _PRIVATE int psrp_builtin_help(int, char *[]);

#ifdef DLL_SUPPORT
// Show DLL orifices which are bound to this application
_PROTOTYPE _PRIVATE int psrp_builtin_show_attached_orifices(int, char *[]);
#endif /* DLL_SUPPORT */

#ifdef PTHREAD_SUPPORT
// Show threads running in PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_show_threads(int, char *[]);

// Start thread running in PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_launch_thread(int, char *[]);

// Pause thread runnin in PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_pause_thread(int, char *[]);

// Pause thread runnin in PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_restart_thread(int, char *[]);

// Kill thread running in PSRP server
_PROTOTYPE _PRIVATE int psrp_builtin_kill_thread(int, char *[]);
#endif /* PTHREAD_SUPPORT */

// Show this applications concurrently held link file locks
_PROTOTYPE _PRIVATE int psrp_builtin_show_link_file_locks(int, char *[]);

// Show this applications flock locks
_PROTOTYPE _PRIVATE int psrp_builtin_show_flock_locks(int, char *[]);

// Show this applications non-default signal handlers
_PROTOTYPE _PRIVATE int psrp_builtin_show_sigstatus(int, char *[]);

// Show this applications signal mask/ pending signal status
_PROTOTYPE _PRIVATE int psrp_builtin_show_sigmaskstatus(int, char *[]);

// Show streams/file descriptors opened by this application
_PROTOTYPE _PRIVATE int psrp_builtin_show_open_fdescriptors(int, char *[]);

// Show (active) children of this application
_PROTOTYPE _PRIVATE int psrp_builtin_show_children(int, char *[]);

// Show slaved interation client channels open
_PROTOTYPE _PRIVATE int psrp_builtin_show_open_sics(int, char *[]);

// Show exit functions registered for this application 
_PROTOTYPE _PRIVATE int psrp_builtin_pups_show_exit_f(int, char *[]);

// Show abort functions registered for this application
_PROTOTYPE _PRIVATE int psrp_builtin_pups_show_abort_f(int, char *[]);

// Show entrance functions registered for this application
_PROTOTYPE _PRIVATE int psrp_builtin_pups_show_entrance_f(int, char *[]);

// Show status of PUPS virtual timers
_PROTOTYPE _PRIVATE int psrp_builtin_pups_show_vitimers(int, char *[]);

// show persistent heaps mapped into process address space
_PROTOTYPE _PRIVATE int psrp_builtin_show_persistent_heaps(int, char *[]);

// show persistent heaps mapped into process address space
_PROTOTYPE _PRIVATE int psrp_builtin_show_htobjects(int, char *[]);

// Load a new dispatch table
_PROTOTYPE _PRIVATE int psrp_builtin_autosave_dispatch_table(int, char *[]);

// Load a new dispatch table
_PROTOTYPE _PRIVATE int psrp_builtin_load_dispatch_table(int, char *[]);

// Save dispatch table
_PROTOTYPE _PRIVATE int psrp_builtin_save_dispatch_table(int, char *[]);

// Reset dispatch table to it initial state */
_PROTOTYPE _PRIVATE int psrp_builtin_reset_dispatch_table(int, char *[]);

// Extend the child table
_PROTOTYPE _PRIVATE int psrp_builtin_extend_chtab(int, char *[]);


#ifdef PERSISTENT_HEAP_SUPPORT
// Attach a persistent heap to PSRP handler
_PROTOTYPE _PRIVATE int psrp_builtin_attach_persistent_heap(int, char *[]);

// Extend persistent heap table
_PROTOTYPE _PRIVATE int psrp_builtin_extend_htab(int, char *[]);
#endif /* PERSISTENT_HEAP_SUPPORT */


#ifdef DLL_SUPPORT
// Attach dynamic function to PSRP handler
_PROTOTYPE _PRIVATE int psrp_builtin_attach_dynamic_function(int, char *[]);

// Extend orifice [DLL] table
_PROTOTYPE _PRIVATE int psrp_builtin_extend_ortab(int, char *[]);
#endif /* DLL_SUPPORT */
...
_PROTOTYPE _PRIVATE int psrp_builtin_display_cache(int, char *[]);

// Extend virtual timer table
_PROTOTYPE _PRIVATE int psrp_builtin_extend_vitab(int, char *[]);

// Extend the file table
_PROTOTYPE _PRIVATE int psrp_builtin_extend_ftab(int, char *[]);

// Overlay PSRP server with new command 
_PROTOTYPE _PRIVATE int psrp_builtin_overlay_server_process(int, char *[]);

// Overfork PSRP server with new command
_PROTOTYPE _PRIVATE int psrp_builtin_overfork_server_process(int, char *[]);

// Attach dynamic function to PSRP handler
_PROTOTYPE _PRIVATE int psrp_builtin_attach_dbag(int, char *[]);
...
{   int i,
    ...
    {  strdate(date);
       (void)fprintf(stderr,
                     "%s %s (%d@%s:%s): PSRP (protocol %4.2F) handler initialised on channel %s/psrp:%s:%s:fifo:IO:%d\n",
                                                                                                                    date,
                                                                                                               appl_name,
                                                                                                                appl_pid, 
                                                                                                               appl_host, 
...
{   int i;
    ...
    {  psrp_object_list_used += PSRP_DISPATCH_PAGE_SIZE;
       ...
       for(i=psrp_object_list_used-PSRP_DISPATCH_PAGE_SIZE; i<psrp_object_list_used; ++i)
...
{   int slot_index,
    ...
    if(appl_verbose == TRUE )
    {  (void)fprintf(stderr,"%s %s (%d@%s:%s): static databag %s attached (at 0x%010x virtual)\n",
                                                                                             date,
                                                                                        appl_name,
                                                                                         appl_pid,
                                                                                        appl_host,
...
{   int  slot_index,
    ...
    do {
           ...
           bag_size += PSRP_BAG_PAGE_SIZE;
           bag_handle     = (_BYTE *)pups_realloc((void *)bag_handle,bag_size);
           bag_bytes_read = pups_pipe_read(bag_des,bag_handle,PSRP_BAG_PAGE_SIZE);
       } while(bag_bytes_read == PSRP_BAG_PAGE_SIZE);
    ...
    if(appl_verbose == TRUE)
    {  (void)fprintf(stderr,"%s %s (%d@%s:%s): dynamic databag %s attached (from %s at 0x%010x virtual)\n",
                                                                                                      date,
                                                                                                 appl_name,
                                                                                                  appl_pid,
                                                                                                 appl_host,
...
{   int  slot_index,
    ...
    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): persistent heap %s attached (from %s at 0x%010x virtual)\n",
                                                                                                      date,
                                                                                                 appl_name,
                                                                                                  appl_pid,
                                                                                                 appl_host,
...
{   int slot_index,
    ...
    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): static function %s attached (at 0x%010x virtual)\n",
                                                                                              date,
                                                                                         appl_name,
                                                                                          appl_pid,
                                                                                         appl_host,
...
{   int  slot_index,
    ...
    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): dynamic function \"%s\" attached (from %s at 0x%010x virtual)\n",
                                                                                                           date,
                                                                                                      appl_name,
                                                                                                       appl_pid,
                                                                                                      appl_host,
...
{   int  i;
    ...
    {  (void)fprintf(psrp_out,"    %d PSRP object slots in use\n",psrp_object_list_size);
       ...
       {  if(psrp_object_list[i].object_handle != (void *)NULL)
          {  (void)fprintf(psrp_out,"    %04d: %-32s (entry at 0x%010x virtual)    ",i,
...
{    int i,
    ...
    {  if(psrp_object_list[i].object_handle != (void *)NULL && (slot_index = psrp_isearch_tag_list(object_tag,i)) >= 0)
       {  
          ...
          {  if(appl_verbose == TRUE)
             {  (void)strdate(date);
                (void)fprintf(stderr,"%s %s (%d@%s:%s): object %s (index %d at 0x%010x virtual) detached\n",
                                                               date,appl_name,appl_pid,appl_host,appl_owner,
...
_PROTOTYPE _PUBLIC void (*on_abort)(void);

_PUBLIC int psrp_register_on_abort(const void *f)
...
{   int i,
    ...
    _BOOLEAN close_psrp_channel = FALSE;
...
(void)fflush(stderr);
    ...
    {  if(appl_verbose == TRUE)
          (void)fprintf(stderr,"%s %s (%d@%s:%s): PSRP (protocol %4.2F) last request repeated %d times\n",
                                                                                                     date,
                                                                                                appl_name,
                                                                                                 appl_pid,
                                                                                                appl_host,
...
#ifdef PSRPLIB_DEBUG
(void)fprintf(stderr,""PSRPLIB SIGCHAN (0x%010x)\n",(unsigned long int)psrp_out);
(void)fflush(stderr);
#endif /* PSRPLIB_DEBUG */
...
{    int idum  = 0,
     ...
     {  int i,
        ...
        /*----------------------------------------------*/
        /* Check that connected clients are still alive */
        /*----------------------------------------------*/
...
_PUBLIC int psrp_chbrk_handler(const unsigned int h_index, const int event)
...
{   int  bytes_out,
    ...
    {  strdate(date);
       ...
       switch(event)
       {    case PSRP_CLIENT_TERMINATED:    if(strcmp(psrp_remote_hostpath[h_index],"notset") == 0)
                                            {  if(psrp_transactions[h_index] == 0)
                                                  (void)fprintf(stderr,
                                                                "%s %s (%d@%s:%s): connection to client (%s) broken on channel \"%s\" (0 client transactons)\n",
                                                                              date,appl_name,appl_pid,appl_host,appl_owner,psrp_client_name[h_index],channel_name);
                                               else if(psrp_transactions[h_index] == 1)
                                                  (void)fprintf(stderr,
                                                  ...
                                                  (void)fprintf(stderr,
                                                               ...
                                                               date,appl_name,appl_pid,appl_host,appl_owner,psrp_client_name[h_index],channel_name,psrp_transactions[h_index]);
                                            }
                                            else
                                            {  if(psrp_transactions[h_index] == 0)
                                                  (void)fprintf(stderr,
                                                                "%s %s (%d@%s:%s): connection to client (%s [via %s]) on channel \"%s\" broken (0 client transactons)\n",
                                                                date,appl_name,appl_pid,appl_host,appl_owner,psrp_client_name[h_index],psrp_remote_hostpath[h_index],channel_name);
                                               else if(psrp_transactions[h_index] == 1)
                                                   (void)fprintf(stderr,
                                                  ...
                                                  (void)fprintf(stderr,
                                                                ...
                                                                date,appl_name,appl_pid,appl_host,appl_owner,psrp_client_name[h_index],psrp_remote_hostpath[h_index],channel_name,psrp_transactions[h_index]);
                                            }
    ...
    {  (void)psrp_clear_client_slot(h_index);
...
_PRIVATE _BOOLEAN psrp_exec_action_object(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_set_mime_type(int argc, char *argv[])
...
_PRIVATE char *r_argv[256] = { (char *)NULL };
...
re_dispatch:
    ...
    if(psrp_exec_action_object(r_argc,r_argv) == TRUE)
       goto object_dispatched;

    (void)fprintf(psrp_out,"    Illegal command\n");
    (void)fflush(psrp_out);
    ...
    if(appl_verbose == TRUE)
    {  strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): PSRP (protocol %4.2F) request \"%s\" not recognised (dispatch failure)\n",
                                                                                                                    date,
                                                                                                               appl_name,
                                                                                                                appl_pid,
                                                                                                               appl_host,
    ...
    if(appl_verbose == TRUE)
    {  (void)fprintf(psrp_out,"\nPSRP (protocol %4.2F) request \"%s\" dispatch failure (%d@%s)\n\n",
...
#ifdef PSRPLIB_DEBUG
(void)fprintf(stderr,""PSRPLIB SIGPSRP3\n");
 (void)fflush(stderr);
#endif /* PSRPLIB_DEBUG */
...
(void)fflush(stderr);
    ...
    {  if(strcmp(psrp_remote_hostpath[c_client],"notset") == 0)
          (void)fprintf(psrp_out,
                         "\nPSRP (protocol %4.2F) request \"%s\" received by %s (%d@%s:%s) from %s (%d@%s)\n\n",
                                                                                          PSRP_PROTOCOL_VERSION,
           ...
           (void)fprintf(psrp_out,
                          "\nPSRP (protocol %4.2F) request \"%s\" received by %s (%d@%s:%s) from %s (%d@%s) [from %s]\n\n",
                                                                                                     PSRP_PROTOCOL_VERSION,
                                                                                                     request_str,appl_name,
                                                                                             appl_pid,appl_host,appl_owner,
                                                                                                psrp_client_name[c_client],
    ...
    {  strdate(date);
       ...
       {  if(req_r_cnt[c_client] > 0)
          ...
          if(strcmp(psrp_remote_hostpath[c_client],"notset") == 0)
             (void)fprintf(stderr,"%s %s (%d@%s:%s): PSRP (protocol %4.2F) request \"%s\" received from %s (%d@%s)\n",
                                                                                                                 date,
                                                                                                            appl_name,
                                                                                                             appl_pid,
                                                                                                            appl_host,
             ...
             (void)fprintf(stderr,"%s %s (%d@%s:%s): PSRP (protocol %4.2F) request \"%s\" received from %s (%d@%s) [from %s]\n",
                                                                                                                           date,
                                                                                                                      appl_name,
                                                                                                                       appl_pid,
                                                                                                                      appl_host,
...
{   int i,
    ...
    char item[SSIZE] = "";
    psrp_object_type *psrp_object = (psrp_object_type *)NULL;
...
_PRIVATE int psrp_builtin_appl_verbose(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_ssh_compress(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_ssh_port(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_transactions(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_autosave_dispatch_table(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_load_dispatch_table(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_save_dispatch_table(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_reset_dispatch_table(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_detach_object(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_hinfo(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_psrp_state(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_psrp_bind_type(int argc, char *argv[])
...
{   if(strcmp("bindtype",argv[0]) != 0) 
    ...
    (void)fprintf(psrp_out,"\n    PSRP server interface (protocol %4.2F) bind status\n\n",PSRP_PROTOCOL_VERSION);
    (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_terminate_process(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_alias(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_clients(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_unalias(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_showaliases(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_new_instance(int r_argc, char *r_argv[])
...
_PRIVATE int psrp_builtin_help(int argc, char *argv[])
...
{   if(strcmp("help",argv[0]) != 0) 
    ...
    (void)fprintf(psrp_out,"\n\n    PSRP request handler version 2.16 (protocol %4.2F)\n",PSRP_PROTOCOL_VERSION);
    ...
    (void)fprintf(psrp_out,"    appl_verbose <on | off>    :   switch server error logging <on> or <off>\n");
    ...
    (void)fprintf(psrp_out,"    nodetach [on | off]        :   if [on] don't detach stdio when in background\n");    
    ...
    (void)fprintf(psrp_out,"\n    P3 server virtual timer control/status functions\n\n");
    ...
    (void)fprintf(psrp_out,"\n    P3 server child control/status functions\n\n");
    (void)fprintf(psrp_out,"    cstat                      :   display active children`\n");
    (void)fprintf(psrp_out,"    chtab       [<n> | shrink] :   display [or set to <n> or Procrustes shrink] number of child table slots\n");
    (void)fprintf(psrp_out,"\n    P3 server command alias control/status functions\n\n");
    ...
    (void)fprintf(psrp_out,"\n    P3 server entry/exit routine control/status functions\n\n");
    ...
    (void)fprintf(psrp_out,"    atrestart                  :   display list of (PUPS) application (CKPT) restart functions\n\n\n");
    (void)fflush(psrp_out);
    ...
    #ifdef BUBBLE_MEMORY_SUPPORT
    (void)fprintf(psrp_out,"    P3 server bubble mememory control/status functions\n");
    (void)fprintf(psrp_out,"    ==================================================\n\n");
    ...
    (void)fprintf(psrp_out,"    hostat <o1> <o2> ... <on>  :   Show statistics for tracked heap object <tracked object>\n");
...
_PRIVATE int psrp_builtin_show_attached_orifices(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_threads(int argc, char *argv[])
...
{   if(strcmp("tstat",argv[0]) != 0)
    ...
    {  int nid;
       ...
       {  (void)fprintf(psrp_out,"expecting integer value (>= 0) for nid\n");
          (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_launch_thread(int argc, char *argv[])
...
{   int i,
    ...
    {  int tpid,
       ...
       (void)fprintf(psrp_out,"Thread %d (LWP: %d, payload function: \"%s\") launched (at 0x%010x virtual)\n",t_index,tpid,tname,(unsigned long int)tid);
       (void)fflush(psrp_out);
    }
    else
...
_PRIVATE int psrp_builtin_pause_thread(int argc, char *argv[])
...
{   pthread_t tid;
    ...
    {  if((tid = pupsthread_tfuncname2tid(argv[1])) == (pthread_t)NULL)
       ...
       {  (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") failed to stop\n",(unsigned long int)tid,argv[1]);
          (void)fflush(psrp_out);

          return(PSRP_OK);
       }

       (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") paused\n",(unsigned long int)tid,argv[1]);
       (void)fflush(psrp_out);
    ...
    {  int i,
       ...
       {  if(ttab[i].tid != (pthread_t *)NULL)
          {  tid = ttab[i].tid;
             ...
             {  (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") stopped\n",(unsigned long int)tid,tmpname);
                (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_restart_thread(int argc, char *argv[])
...
{   pthread_t tid;
    ...
    {  if((tid = pupsthread_tfuncname2tid(argv[1])) == (pthread_t)NULL)
       ...
       {  (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") failed to restart\n",(unsigned long int)tid,argv[1]);
          (void)fflush(psrp_out);

          return(PSRP_OK);
       }

       (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") restarted\n",(unsigned long int)tid,argv[1]);
       (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_kill_thread(int argc, char *argv[])
...
{   pthread_t tid;
    ...
    {  if((tid = pupsthread_tfuncname2tid(argv[1])) == (pthread_t)NULL)
       ...
       {  (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") failed to terminate\n",(unsigned long int)tid,argv[1]);
          (void)fflush(psrp_out);

          return(PSRP_OK);
       }
    
       (void)fprintf(psrp_out,"Thread (0%x, name \"%s\") terminated\n",(unsigned long int)tid,argv[1]);
       (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_show_procstatus(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_link_file_locks(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_flock_locks(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_sigstatus(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_sigmaskstatus(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_open_fdescriptors(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_children(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_caches(int argc, char *argv[])
...
{   if(strcmp("cachestat",argv[0]) != 0)
    ...
    {  int c_index; 
       ...
       if(cache_display_statistics(psrp_out,c_index) == (-1))
...
/*------------------------------------------------------------------------------------------
    ...
    from hstat <heap> show the object map of that heap ...
------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_show_persistent_heaps(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_open_sics(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_pups_show_entrance_f(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_pups_show_exit_f(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_pups_show_abort_f(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_pups_show_vitimers(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_htobjects(int argc, char *argv[])
...
{   int i,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&htab_mutex);
    #endif /* PTHREAD_SUPPORT */
...
(void)fflush(stderr);
       ...
       {  
          ...
          (void)fprintf(stream,"    %04d: \"%-32s\"  mapped at 0x%010x virtual (size 0x%010x bytes)\n",i,
                                                                                          htable[i].name,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_unlock(&htab_mutex);
    #endif /* PTHREAD_SUPPORT */
       ...
       {  alias_index = psrp_isearch_tag_list(basename,i);
         ...
         {  (void)fprintf(psrp_out,"\n\n    Aliases list containing object tag \"%s\"\n\n",basename); 
            for(j=0; j<psrp_object_list[i].aliases_allocated; ++j)
               if(psrp_object_list[i].object_tag[j] != (char *)NULL)
               {  if(j == 0)
                    (void)fprintf(psrp_out,"%8d:    %s (root tag of PSRP object at 0x%010x virtual)\n",j,
                                                                       psrp_object_list[i].object_tag[j],
                                                                       psrp_object_list[i].object_handle);
                  else
                     (void)fprintf(psrp_out,"%8d:    %s\n",j,psrp_object_list[i].object_tag[j]);
               }
...
_PRIVATE int psrp_builtin_attach_dbag(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_attach_persistent_heap(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_attach_dynamic_function(int argc, char *argv[])
...
{   int i;
    ...
    {  if(psrp_client_pid[i] > 0)
       {  (void)fprintf(psrp_out,"%d: %s (%d@%s)",i,psrp_client_name[i],psrp_client_pid[i],psrp_client_host[i]);
          ...
          if((void *)psrp_client_exitf[i] != (void *)NULL)
             (void)fprintf(psrp_out,"    (exit function %s installed at 0x%010x virtual)",
                                                                    psrp_client_efname[i],
                                                           (long int)psrp_client_exitf[i]);
    ...
    if(n_clients == 1)
       (void)fprintf(psrp_out,"\n\n    Total of 1 client currently attached to server\n\n");
    else
...
/*------------------------------------------------------------------------------
    Set current PSRP interaction channel -- for interactive abort ...
------------------------------------------------------------------------------*/
...
_PRIVATE int psrp_builtin_overlay_server_process(int argc, char *argv[])

{   int  i;

    char overlay_command_name[4096] = "",
         overlay_command_tail[4096] = "";
...
_PRIVATE int psrp_builtin_overfork_server_process(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_extend_ftab(int argc, char *argv[])
...
{   int i,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&ftab_mutex);
    #endif /* PTHREAD_MUTEX */
    ...
    if(n_files < appl_max_files || strcmp(argv[1],"shrink") == 0)
    {
       ...
       {  if(ftab[i].fdes != (-1))
          {  int j;
             ...
             #ifdef PTHREAD_SUPPORT
             (void)pupsthread_mutex_unlock(&ftab_mutex);
             #endif /* PTHREAD_MUTEX */
...
_PRIVATE int psrp_builtin_extend_vitab(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_extend_chtab(int argc, char *argv[])
...
{   int i,
    ...
    #ifdef PTHREAD_SUPPORT
    (void)pupsthread_mutex_lock(&chtab_mutex);
    #endif /* PTHREAD_SUPPORT */
    ...
    if(n_children < appl_max_child || strcmp(argv[1],"shrink") == 0)
    {
       ...
       {  if(chtab[i].pid != (-1))
          {  int j; 
             ...
             #ifdef PTHREAD_SUPPORT
             (void)pupsthread_mutex_unlock(&chtab_mutex);
             #endif /* PTHREAD_SUPPORT */
...
_PRIVATE int psrp_builtin_extend_htab(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_extend_ortab(int argc, char *argv[])
...
/*----------------------------------------------------------------------------
    Check process circadian activity schedule ...
----------------------------------------------------------------------------*/
...
cronslot_found:
    ...
    {  (void)strdate(date);
       if(crontab[ctab_index].func  == (void *)NULL)
          ...
          (void)fprintf(stderr,"%s %s (%d@%s:%s): [crontab slot %d]  %s (at 0x%010x virtual) scheduled between %s and %s\n\n",
...
{   int i,
    ...
    {  if(crontab[i].from > 0)
       {  if(crontab[i].func != (void *)NULL)
             (void)fprintf(stream,"    %04d: (payload %-48s at 0x%010x virtual) start: %-32s, stop %-32s\n",
                                                                                                          i,
                                                                                           crontab[i].fname,
                                                                                            crontab[i].func,
                                                                                        crontab[i].fromdate,
...
_PRIVATE int psrp_builtin_crontab_schedule(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_crontab_unschedule(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_crontab(int argc, char *argv[])
...
/*--------------------------------------------------------------------------------------------
    Dynamically unset PSRP servers system migration contexct ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_set_nodetach(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_show_rusage(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_set_rlimit(int argc, char *argv[])
    ...
    {  int vm_size;
       ...
       {  buf.rlim_cur = buf.rlim_max;
          ...
          if(buf.rlim_max == RLIM_INFINITY)
             ...
             (void)fprintf(psrp_out,"\n(maximum) virtual memory size is set to hard limit (%d Mbytes)\n\n",buf.rlim_max/1000000);
          (void)fflush(psrp_out);
       ...
       {  if(buf.rlim_max != RLIM_INFINITY && 1000000*vm_size > buf.rlim_max)
          {  (void)fprintf(psrp_out,"\nhard limit for virtual memory size is: %d Mbytes\n",buf.rlim_max/1000000);
             (void)fflush(psrp_out);

             ++cnt;
          }
          else
          {  (void)fprintf(psrp_out,"\n(maximum) virtual memory size set to: %d Mbytes\n\n",vm_size);
             (void)fflush(psrp_out);
...
/*--------------------------------------------------------------------------------------------
    Dynamically set process to exit if its effective parent exits  ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_set_pexit(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_set_unpexit(int argc, char *argv[])
...
/*--------------------------------------------------------------------------------------------
    Dynamically set PSRP servers system migration context ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_set_rooted(int argc, char *argv[])
...
{   if(strcmp("rooted",argv[0]) != 0)
    ...
    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): PSRP server is rooted (system context cannot migrate\n)",
                                                           date,appl_name,appl_pid,appl_host,appl_owner);
       (void)fflush(stderr);
    }

    (void)fprintf(psrp_out,"\nPSRP server is rooted (system context cannot migrate)\n\n");
    (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_set_unrooted(int argc, char *argv[])
...
{   if(strcmp("unrooted",argv[0]) != 0)
    ...
    if(appl_verbose == TRUE)
    {  (void)strdate(date);
       (void)fprintf(stderr,"%s %s (%d@%s:%s): PSRP server is not rooted (system context can migrate\n)",
                                                            date,appl_name,appl_pid,appl_host,appl_owner);
       (void)fflush(stderr);
    }

    (void)fprintf(psrp_out,"\nPSRP server is not rooted (system context can migrate)\n\n");
    (void)fflush(psrp_out);
...
/*--------------------------------------------------------------------------------------------
    Dynamically set PSRP servers current working directory ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_set_parent(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_set_cwd(int argc, char *argv[])
...
/*--------------------------------------------------------------------------------------------
    Dynamically protect process thread of execution ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_set_trys(int argc, char *argv[])
...
/*--------------------------------------------------------------------------------------------
    Dynamically protect a file or files) ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_file_live(int argc, char *argv[])
...
/*--------------------------------------------------------------------------------------------
    Dynamically unprotect a file or files) ...
--------------------------------------------------------------------------------------------*/

_PRIVATE int psrp_builtin_file_dead(int argc, char *argv[])
...
/*----------------------------------------------------------------------------------------
    Fork a PUPS server -- this routine will build -- all open files etc are inherited
...
_PRIVATE int received(int signum)

{   

#ifdef PSRPLIB_DEBUG
(void)fprintf(stderr,"PSRPLIB RECV: %d\n",signum);
(void)fflush(stderr);
#endif /*PSRPLIB_DEBUG */
...
_PRIVATE int psrp_builtin_set_secure(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_set_mbubble_utilisation_threshold(int argc, char *argv[])
...
{   int mbubble_utilisation;
    ...
    {  (void)fprintf(psrp_out,"\nusage: ckset [-usage] | <minimum bubble utilisation %%>\n\n");
       (void)fflush(psrp_out);
...
_PRIVATE int psrp_builtin_show_malloc_stats(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_ssave(int argc, char *argv[])
...
_PRIVATE int psrp_builtin_set_vitimer_quantum(int argc, char *argv[])
...
{   char inc_command[SSIZE]      = "",
    ...
    (void)snprintf(mhn_command,SSIZE,"csh -c \"mhn +%s %s |& grep Subject\"",appl_mdir,msg_file);
...
{   char mhn_command[SSIZE] = "",
    ...
    (void)snprintf(mhn_command,SSIZE,"csh -c \"mhn +%s %s |& grep \"Replyto:\"\"",appl_mh_folder,msg_file);
    if((pstream = popen(mhn_command,"r")) == (FILE *)NULL) 
    {  pups_set_errno(ENOEXEC);
       return(-1);
    }
    ...
    (void)snprintf(mhn_command,SSIZE,"csh -c \"mhn +%s %s |& grep \"From:\"\"",appl_mh_folder,msg_file);
    if((pstream = popen(mhn_command,"r")) == (FILE *)NULL)
    {  pups_set_errno(ENOEXEC);
       return(-1);
    }
...
{   int i,
    ...
    {  
       ...
       (void)execlp("csh","csh","-c",ssh_cmd,(char *)0);
