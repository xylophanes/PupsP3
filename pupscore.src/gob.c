...
static int gob_exit(int signum)
...
static int gob_fifo_homeostat(int signum)
    ...
    {  if(strcmp(argv[i],"-verbose") == 0)
       ...
       {  if(fork() == 0)
          {  int  j,
             ...
             (void)strcpy(args[0],argv[i+1]);
             for(j=1; j<argc; ++j)
             {  if(j != i && j != i+1)
                {  (void)strcpy(args[cnt],argv[j]);
       ...
       {  if(i == argc -1 || argv[i+1][0] == '-')
          ...
          (void)strcpy(gob_fifo_name,argv[i+1]);
          ++i;
          decoded += 2;
       }
    }
