...
(void)fflush(psrp_out);
   ...
   if(replys != (char **)NULL)
      (void)xafree(n_requests,(void *)replys);
...
{   (void)fprintf(stderr,"int app (PSRP) embryo %s: [ANSI C, PUPS MTD D]\n",EMBRYO_VERSION);
    ...
    if(level > 1)
    {  (void)fprintf(stderr,"(C) 1996-2019 Tumbling Dice\n");
...
{   int t_index,
    ...
    (void)pupsthread_mutex_lock(&test_mutex);
...
{   int  i,
    ...
    do {   (void)pups_fgets(item,SSIZE,pipestream);
...
{   int cnt = 0;
    ...
    while(1)
    {    (void)fprintf(stderr,"in ASCII (%d)\n",cnt);
         (void)fflush(stderr);

         (void)fprintf(psrp_out,"ASCII text (%d)\n",cnt);
         (void)fflush(psrp_out);
